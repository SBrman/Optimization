...
def hw4_1():
    def f(x):
        return -500*x[1] - 250*x[2] - 600*x[3]

    A = np.array([[2, 1, 1, 1, 0, 0],
                  [3, 1, 2, 0, 1, 0],
                  [1, 2, 4, 0, 0, 1]])
    b = np.array([240, 150, 180]).reshape((3, -1))
...
def get_soln(A, b):
    ...
    for columns in combinations(range(n), m):
        ...
        N = set(range(1, n+1)) - B
        x_B = {column: sympy.Rational(Fraction(float(value)).limit_denominator())
               for column, value in zip(B, xB)}
